...
### 🪪 Life Passport｜人生护照
**定位**：为一次真实的人生抵达盖下印章。
*   当前内部版本为 `Passport v0.2.2`，使用单页工作台完成记录、装帧、实时预览、本地保存与纪念卡下载。
*   提供十二类人生主题、手绘日期选择、可选照片及多种印章与照片质感。
*   照片预览保留原始比例，纸张边缘会随内容尺寸自适应。
*   页面与导出海报共用本地手写字体，章印预览和导出结果保持一致。
*   通过版本化 IndexedDB 在当前浏览器保存默认护照、印章与压缩照片，并提供最小护照列表。
*   已保存印章支持详情查看、编辑更新、删除确认与再次导出。
*   Schema v2 提供容量预检、版本迁移与孤立图片/照片标记自动修复。
*   本地数据不承诺跨设备同步或永久保存；不承诺登录、同步或云端存储。
