...
const SOURCE_DEST_DIR = path.join(process.cwd(), 'src/assets/weather-icons');
const PUBLIC_DEST_DIR = path.join(process.cwd(), 'public/weather-icons');
...
function processIcons() {
  [SOURCE_DEST_DIR, PUBLIC_DEST_DIR].forEach((dir) => {
  ...
  iconCodes.forEach(code => {
    ...
    if (fs.existsSync(srcPath)) {
      ...
      if (svgTagMatch) {
        ...
        content = content.replace(/viewBox="0 0 16 16"/, 'viewBox="-1.5 -1.5 19 19"');
        
        fs.writeFileSync(path.join(SOURCE_DEST_DIR, `${code}.svg`), content);
        fs.writeFileSync(path.join(PUBLIC_DEST_DIR, `${code}.svg`), content);
  ...
  console.log(`Generated static weather icons at ${SOURCE_DEST_DIR} and ${PUBLIC_DEST_DIR}`);
}
